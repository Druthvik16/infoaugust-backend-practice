...
let attachmentSheetName = 'Working File'
let status;
let userId;
let loggedUserUuid;
let categoryName = 'Credit Note';
let documentCategoryId;
let s3Folder = 'Input_Working_Raw_Sap_dump'
...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        form.parse(req, async function (error, fields, files) 
        {
            ...
            fileObject.workingFile.forEach(file => {
            ...
            {
                uploadFileOnS3Bucket(fileObject.workingFile, 0, fileObject.workingFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
            else
            {
                uploadFileOnLocal(fileObject.workingFile, 0, fileObject.workingFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
        })  
    } 
    catch(e)
    {
        ...
        return res.json({
            "status_code" : 500,
            "message" : "Credit Note Working Upload Failed",
