...
let userId;
let loggedUserUuid;
let categoryName = 'Credit Note';
let documentCategoryId;
let s3Folder = 'Input_Summary_Raw_Sap_dump'
...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        form.parse(req, async function (error, fields, files) 
        {
            ...
            fileObject.summaryFile.forEach(file => {
                console.log("path.extname(file.originalFilename)",path.extname(file.originalFilename))
            ...
            console.log(fileStoreTo)  
            if(fileStoreTo == 'S3BUCKET')
            {
                uploadFileOnS3Bucket(fileObject.summaryFile, 0, fileObject.summaryFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
            else
            {
                uploadFileOnLocal(fileObject.summaryFile, 0, fileObject.summaryFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
        })  
    } 
    catch(e)
