...
let userId;
let loggedUserUuid;
let s3Folder = 'Input_Invoice_Summary_Raw_Sap_dump'
let categoryName = 'Invoice';
...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        form.parse(req, async function (error, fields, files) 
        {
            ...
            fileObject.invoiceFile.forEach(file => {
            ...
            {
                uploadFileOnS3Bucket(fileObject.invoiceFile, 0, fileObject.invoiceFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
            else
            {
                uploadFileOnLocal(fileObject.invoiceFile, 0, fileObject.invoiceFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
        })  
    } 
    catch(e)
    {
        ...
        return res.json({
            "status_code" : 500,
            "message" : "Invoice Summary Upload Failed",
                    ...
                    uniqueFunction.encryptFileBuffer(file.filepath, file.originalFilename,null, null, 'file').then(encryptedData => {
                        if(encryptedData?.result)
                        {
                            uniqueFunction.uploadFiles(encryptedData?.file, file.originalFilename, clientUuid, s3Folder).then(async(uploadFiles) => 
                            {
                                ...
                                {
                                    db.saveUploadDocLogMaster(file.originalFilename, clientId, documentAttachmentId, status, new Date(), uploadFiles.s3FilePath,documentCategoryId, encryptedData?.encriptionKey, encryptedData?.encriptionIV).then(saveMaster => {
                                        start++
                                        uploadFileOnS3Bucket(fileObject, start, end, clientUuid, masterId, res, clientId, documentAttachmentId, status,documentCategoryId, rejectedFiles)
                                    })
                                }
        ...
        if(start < end)
        {
            ...
            uniqueFunction.unquieName(identifierName, ['file_name'],{  "file_name" : file.originalFilename }, id, 0).then(uniqueCheckName => {
                ...
                // { 
                    ...
                    uniqueFunction.encryptFileBuffer(file.filepath, file.originalFilename,null, null, 'file').then(async(encryptedData) => {
                        if(encryptedData?.result)
                        {
                            let saveDataTransactLog = await db.saveDataTransactLog('UP', 'EU', '', '', file?.size, apiName, 'LOC', new Date(), clientId, file.originalFilename)
                            uniqueFunction.singleFileUpload(encryptedData?.file, getPath.getName('documentFolders'), file.originalFilename, (clientUuid + '/' + s3Folder)).then(uploadFiles => 
                            {
                                if(uploadFiles && uploadFiles.result == true)
                                {
