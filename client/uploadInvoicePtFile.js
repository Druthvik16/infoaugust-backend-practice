...
let attachmentSheetName = 'PT File'
let status;
let userId;
let loggedUserUuid;
let s3Folder = 'Input_Invoice_Pt_File_Raw_Sap_dump'
...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        form.parse(req, async function (error, fields, files) 
        {
            ...
            fileObject.invoicePtFile.forEach(file => {
            ...
            {
                uploadFileOnS3Bucket(fileObject.invoicePtFile, 0, fileObject.invoicePtFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
            else
            {
                uploadFileOnLocal(fileObject.invoicePtFile, 0, fileObject.invoicePtFile?.length, clientUuid, 0, res, clientId, documentAttachmentId, status, documentCategoryId, [])
            }
        })  
    } 
    catch(e)
    {
        ...
        return res.json({
            "status_code" : 500,
            "message" : "Invoice PT File Upload Failed",
