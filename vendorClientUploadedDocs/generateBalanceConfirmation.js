...
module.exports = express.Router().post('/', async (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        let validatedVendorsLedger = vendorsLedger.map((ledger) => {
            ...
            // if (!ledger['Balance confirmation date']) remark += 'Missing Balance confirmation date; ';
            if (!ledger['Amount'] && Number(ledger['Amount']) != 0) remark += 'Missing Amount; ';
            // else if (isNaN(Number(ledger['Amount'])) || Number(ledger['Amount']) <= 0) remark += 'Amount should be greater than zero; ';

            const currentDate = new Date();
            let balanceConfirmationDate;

            if (!ledger['Balance confirmation date']) {
                remark += 'Missing Balance confirmation date; ';
            } else {
                if (typeof ledger['Balance confirmation date'] === 'number') {
                    // Convert Excel serial date to JavaScript date
                    balanceConfirmationDate = new Date((ledger['Balance confirmation date'] - 25569) * 86400 * 1000);
                } else {
                    // Parse the date as a string
                    balanceConfirmationDate = new Date(ledger['Balance confirmation date']);
                }

                // Check if the date is invalid or in the future
                if (isNaN(balanceConfirmationDate.getTime())) {
                    remark += 'Invalid Balance confirmation date; ';
                } else if (balanceConfirmationDate > currentDate) {
                    remark += `Balance confirmation date cannot be greater than today's date; `;
                }
            }
        ...
        if (vendorsWithoutRemarks.length === 0) {
            return res.status(400).json({
                ...
                status_name: getCode.getStatus(400),
                data: {
                    file: xlsxFile, fileName: files.uploadedFile.originalFilename
                }
            });
        }
...
async function processVendor(vendorLedgers, start, end, documentCategoryId, client, documentId, documentAttachmentId, vendorResult, vendorToMail) {
    try {
        if (start < end) {
            ...
            let amount = vendorLedger['Amount'];
            let uploadedOn = new Date();

            let balanceConfirmationDate = vendorLedger['Balance confirmation date'];
...
async function balanceConfirmationInitiatedTemplate(vendors, clientName) {
    try {
        ...
        let balanceConfirmationDate = vendors[0]['Balance confirmation date'];
        const actualDate = formatDate.addDays(epochDate, balanceConfirmationDate - 2);
        console.log(actualDate, vendors[0]['Balance confirmation date'], vendors)
