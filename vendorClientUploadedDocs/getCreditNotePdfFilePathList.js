...
const logFilePath = getPath.getName('vendor/s3/creditNote/pdf')
...
module.exports = require('express').Router().get('/',async(req,res) => 
{
    ...
    { 
        ...
        {
            await uniqueFunction.writeLogIntoFile({"creditNotePdfFilePaths" : []}, logFileName, "Get Credit Note Pdf List", logFilePath, 'green')
            res.status(200)
            return res.json({
                "status_code" : 200,
                "message"     : "success",
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Credit Note Pdf List", logFilePath, 'red')
        console.log("e************", e)
        res.status(500)
        return res.json({
            "status_code" : 500,
...
async function getListObjectFromLocal(clients, start, end, res, fileList)
{
    try
    {
        if(start < end)
        {
            ...
            fileLists.forEach(file => {               
                ...
                if(path.extname(fileName)?.toLowerCase() != '.pdf')
                {
                    ...
                    uniqueFunction.writeLogIntoFile("File type not accepted : " + path.extname(fileName), logFileName, "Get Credit Note Pdf List: "+ file?.Key, logFilePath, 'red')
        ...
        {
            await uniqueFunction.writeLogIntoFile({"creditNotePdfFilePaths" : fileList}, logFileName, "Get Credit Note Pdf List", logFilePath, 'green')
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Credit Note Pdf List => getListObjectFromLocal", logFilePath, 'red')
        console.log(e)
        start++;
        getListObjectFromLocal(clients, start, end, res,fileList)
    }
