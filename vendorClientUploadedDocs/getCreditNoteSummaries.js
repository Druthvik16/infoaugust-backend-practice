...
module.exports = require('express').Router().get('/:clientUuid?/:vendorUuid?/:fromDate?/:toDate?/:action?',async(req,res) => 
{
    try
    {
        ...
        let getLastDocumentUploadOn = await db.getLastDocumentUploadDate(clientUuid, 'Credit Note', vendorUuid, action)
        let uploadedOn = getLastDocumentUploadOn[0].uploadedOn
        console.log(uploadedOn)
        getCreditNoteSummaries = await db.getCreditNoteSummaries(vendorUuid, clientUuid, fromDate, toDate, action, uploadedOn)
