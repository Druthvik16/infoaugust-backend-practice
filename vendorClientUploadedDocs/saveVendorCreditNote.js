    ...
    try {
        ...
        if (req.headers['content-type'] && req.headers['content-type'].includes('multipart/form-data')) {
            ...
            form.parse(req, async function (error, fields, files) {
                try {
                    if (error) {
                        uniqueFunction.removeFileFromDirectory(files?.creditNoteFile?.filepath)
                        res.status(400)
                        return res.json({
                            "status_code": 400,
                            "message": error?.stack,
                    ...
                    if (!req.body.vendor || !JSON.parse(req.body.vendor)?.uuid || !req.body.invoiceNumber || !req.body.creditNoteNumber || !req.body.amount || !req.body.creditNoteDate || !req.body.description) {
                        uniqueFunction.removeFileFromDirectory(files?.creditNoteFile?.filepath)
                        res.status(400)
                        return res.json({
                            "status_code": 400,
                    ...
                    let billNoOrRefNo = req.body.creditNoteNumber[0]
                    let invoiceNumber = req.body.invoiceNumber[0]
                    let postingDate = req.body.creditNoteDate[0]
                    let narration = uniqueFunction.manageSpecialCharacter(req.body.description[0])
                    let creditAmount = req.body.amount[0]
                    let filePath = null
                    ...
                    if (saveVendorUploadedDocMaster.affectedRows > 0) {
                        ...
                        let encryptedData = await uniqueFunction.encryptFileBuffer(files.creditNoteFile.filepath, files.creditNoteFile.originalFilename, null, null, 'file')
                        if (encryptedData?.result) {
                            let uploadFileToS3Bucket = await uniqueFunction.uploadVendorFiles(encryptedData?.file, files.creditNoteFile.originalFilename, clientUuid, documentNewFolderPath, 'vendor')
                            if (uploadFileToS3Bucket && uploadFileToS3Bucket?.result) {
                                let saveDataTransactLog = await db.saveDataTransactLog('UP', 'SU', '', '', files.creditNoteFile?.size, apiName, 'S3', new Date(), clientUuid, files.creditNoteFile?.originalFilename)

                                let savePdfFile = await db.saveCreditNotePdfFile(files.creditNoteFile.originalFilename, uploadFileToS3Bucket.s3FilePath, saveVendorUploadedDocMaster.insertId, 2, new Date(), encryptedData?.encriptionKey, encryptedData?.encriptionIV)
                                
                                let sql = `UPDATE client_vendor_upload_doc_log_master SET status = 'Completed', completed_on = ? WHERE UPPER(file_name) = UPPER('${files.creditNoteFile.originalFilename}')`
                                uniqueFunction.removeFileFromDirectory(files.creditNoteFile.filepath)
