...
let logFilePath = getPath.getName('vendor/s3/form16/pdf')
...
module.exports = require('express').Router().post('/',async(req,res) => 
{
    ...
    { 
        ...
        {
            await uniqueFunction.writeLogIntoFile("Provide all values : FilePath Missing", logFileName, "Get Form 16 Pdf File", logFilePath, 'red')
            res.status(400)
            return res.json({
                "status_code" : 400,
    ...
    {
        console.log("e************", e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Form 16 Pdf File Catch", logFilePath, 'red')
        res.status(500)
        return res.json({
            "status_code" : 500,
            "message"     : "Error",
...
async function getFileFromLocal(filePath, res)
{
    ...
    {
        console.log("e************", e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Form 16 Pdf File Catch (local)", logFilePath, 'red')
        res.status(500)
        return res.json({
            "status_code" : 500,
            "message"     : "Error",
...
async function sendFile(filePath, fileObj, res)
{
    ...
    { 
        ...
        {
            await uniqueFunction.writeLogIntoFile("Encryption key not found for file", logFileName, "Get Form 16 Pdf File : " + fileName, logFilePath, 'red')
            res.status(500)
            return res.json({
                "status_code" : 500,
        ...
        else
        {
            ...
            if(decryptedData?.result)
            {
                ...
                await uniqueFunction.writeLogIntoFile("File Downloaded by Bot Process", logFileName, "Get Form 16 Pdf File : " + fileName, logFilePath, 'green')
            ...
            {
                await uniqueFunction.writeLogIntoFile("File Not Decrypted", logFileName, "Get Form 16 Pdf File : " + fileName, logFilePath, 'red')
                res.status(500)
                return res.json({
                    "status_code" : 500,
