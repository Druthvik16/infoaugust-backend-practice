...
module.exports = require('express').Router().get('/:id',async(req,res) => 
{
    try
    {
        ...
        id = req.params.id
        apiName = req.baseUrl
        getFilePath = await db.getVendorUploadedFilePath(id)
        ...
        else
        {
            ...
            s3.getObject(params,async function(err, data) 
            {
                ...
                {
                    let saveDataTransactLog = await db.saveDataTransactLog('DN', 'EU', getFilePath[0]?.partnerId, getFilePath[0]?.locationId, data?.ContentLength, apiName, 'S3', new Date(), getFilePath[0].filePath.split('/')[3], fileName)
