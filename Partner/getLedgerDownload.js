...
module.exports = require('express').Router().get('/:financialYearId?/:clientUuid?/:partnerUuid?/:fromDate?/:toDate?/:customerCodes?', async (req, res) => {
    try {
        ...
        {
            partner = await db.getPartnerData(partnerUuid);
            if (partner?.length == 0) {
                res.status(400)
                return res.json({
                    "status_code": 400,
        ...
        {            
            partner = await db.getPartnerUserData(partnerUuid);
            if (partner?.length == 0) {
                res.status(400)
                return res.json({
                    "status_code": 400,
        ...
        if(!customerCodes || customerCodes?.length == 0)
        {
            ...
            if(loggedUserType != 'Partner-User')
            {
                partnerLocations = await db.getPartnerlocationCodeOfPartner(partnerId)
            }
            else
            {
                partnerLocations = await db.getPartnerlocationCodeOfPartnerUser(partnerId)
            }
            customerCodes = partnerLocations.map(item => item.code).join(',')
        }
            ...
            s3.getObject(params, async function (err, data) {
                ...
                `;
                   ...
                   const openingBalances = alasql(sqlSelectOpeningBalance, [unionOpeningBalances]);
                    ...
                    // console.log(filteredData?.length, openingBalances);
