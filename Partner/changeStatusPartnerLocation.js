...
module.exports = require('express').Router().get('/:uuid',async(req,res) => 
{
    try
    {
        ...
        const currentData = await db.getPartnerLocationDataByUuid(uuid)
        ...
        const changeStatusPartnerLocation = await db.changeStatusPartnerLocation(uuid)
        if(changeStatusPartnerLocation.affectedRows > 0)
        {
            ...
            const saveActivityLog = await commonDb.saveInfoaugustActivityLog(userId, loggedUserType, loggedUserTable, 'Partner Location Profile Updated', uniqueFunction.manageSpecialCharacter(logChanges.join(' ,')), currentRecord.id, 'partner_location_detail', null, 'success')
            res.status(200)
            return res.json({
                "status_code" : 200,
                "message"     : "success",
