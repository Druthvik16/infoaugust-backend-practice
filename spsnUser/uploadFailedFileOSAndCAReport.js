...
module.exports = require('express').Router().post('/',async(req,res) => 
{
    try
    {
        ...
        form.parse(req, async function (error, fields, file) 
        {
            ...
            let encryptKey = await db.getEncryptionDataForSpsn(fileObject.uploadFile.originalFilename)
            ...
            else
            {
                ...
                {
                    let uploadFileToS3Bucket = await uniqueFunction.uploadSpsnFiles(encryptedData?.file, fileObject.uploadFile.originalFilename, clientUuid, documentFailedFolderPath)
                    if(uploadFileToS3Bucket && uploadFileToS3Bucket?.result)
                    {
                        let saveDataTransactLog = await db.saveDataTransactLog('UP', 'SU', 0, 0,fileObject.uploadFile?.size, apiName, 'S3', new Date(), clientUuid, fileObject.uploadFile.originalFilename)

                        let sql = `UPDATE client_spsn_upload_doc_log_master SET failed_on = ?, failed_file_path = '${uploadFileToS3Bucket?.s3FilePath}' WHERE UPPER(file_name) = '${fileObject.uploadFile.originalFilename?.toUpperCase()}'`
