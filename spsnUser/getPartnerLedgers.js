...
module.exports = require('express').Router().get('/:spsnUuid?/:partnerUuid?/:partnerLocationUuid?/:clientUuid?/:fromDate?/:toDate?/:dateType?/:selectedSpsnUuid?', async (req, res) => {
    try {
        ...
        let getLastDocumentUploadOn = await db.getLastDocumentUploadDate(clientUuid, 'Ledger', partnerUuid, partnerLocationUuid, '', spsnUuid, userTypeCode, selectedSpsnUuid)
        ...
        const getPartnerLedgers = await db.getPartnerLedgers(partnerUuid, partnerLocationUuid, clientUuid, fromDate, toDate, null, spsnUuid, uploadedOn, userTypeCode, dateType, selectedSpsnUuid)
        res.status(200)
        return res.json({
            "status_code": 200,
            "message": "success",
            "data": { "partnerLedgers": getPartnerLedgers, lastUploadedOn },
