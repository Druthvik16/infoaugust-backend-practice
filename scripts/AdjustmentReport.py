...
def getDocumentCategoryId():
    filterDocumentCategories = [documentCategory['id'] for documentCategory in documentCategories if "CA" == documentCategory["code"]]
...
def getDocumentId():
    filterDocuments = [document['id'] for document in documents if "CA" == document["code"]]
...
def saveSuccessFile(fileName, fileObj, monthName, yearName, documentCategoryId, documentId, documentAttachmentId, financialYearId, uploadDate, clientUUID):
...
"client" : json.dumps({"uuid" : clientUUID}),
"monthPeriod": monthName + '-' + yearName,
...
if mode == 'dev':
    ...
    financialYearId = 1
    inputFilePath = r'C:\Divyansh\APProcess\Adjustment Report\InputFile_30-11-2024.xlsx'
    accountSPSNFilePath = r'C:\Divyansh\APProcess\Adjustment Report\partnerLocation.txt'
    spsnFilePath = r'C:\Divyansh\APProcess\Adjustment Report\spsn.txt'
    documents = [
  {
    "id": 1,
...
monthName = calendar.month_name[int(uploadDate[1])]
yearName = uploadDate[2]
uploadDate.reverse()
...
isSuccess = False
list_FailedDF = []
requiredColumns = ["Customer Number", "SPSN", "Clearing Doc Date", "Debit Document Date", "Credit Document Date"]
try:
    ...
    #Convert date format
    df['Clearing Doc Date'] = pd.to_datetime(df['Clearing Doc Date'])
    df['Clearing Doc Date'] = df['Clearing Doc Date'].dt.strftime("%Y-%m-%d")
    df['Debit Document Date'] = pd.to_datetime(df['Debit Document Date'])
    df['Debit Document Date'] = df['Debit Document Date'].dt.strftime("%Y-%m-%d")
    df['Credit Document Date'] = pd.to_datetime(df['Credit Document Date'])
    df['Credit Document Date'] = df['Credit Document Date'].dt.strftime("%Y-%m-%d")
    ...
    for group in gdf.groups:
        ...
        for index, row in df_OutstandingReport.iterrows():
            Remark = validateAccountSPSN(row['Customer Number'], row['SPSN'])
            ...
            if spsnUUID == '':
                ...
                list_FailedDF.append(row)
            elif pd.to_datetime(row['Clearing Doc Date']).month != pd.to_datetime(uploadDate).month:
                #Write row to failed excel file
                # print(json.dumps({"rowDate":pd.to_datetime(row['Clearing Doc Date']).month, "uploadDate" : pd.to_datetime(uploadDate).month, "message": "Month not matched"}))
                row['Process Remark'] = 'Month not matched'
                list_FailedDF.append(row)
            elif pd.to_datetime(row['Clearing Doc Date']) > pd.to_datetime(uploadDate):
                #Write row to failed excel file
                # print(json.dumps({"rowDate":pd.to_datetime(row['Clearing Doc Date']), "uploadDate" : pd.to_datetime(uploadDate), "message": "Date exceed"}))
                row['Process Remark'] = 'Date exceedDate exceed'
                list_FailedDF.append(row)
            else:
    ...
    if len(list_SuccessDF) > 0:
        ...
        jsonFileName = 'CA_' + financialYear + '.json'
        ...
        statusCode = saveSuccessFile(outputFileName, dfile, monthName, yearName, documentCategoryId, documentId, documentAttachmentId, financialYearId, uploadDate, clientUUID)
        dfile.close()
    ...
    print(json.dumps("From " + str(len(df)) + " adjustment report " + str(successCount) + " rows acceptd and "  + str(len(list_FailedDF)) + " rows rejected"))
    if len(list_FailedDF) > 0:
        #Write all failed rows to Excel file
        outputPath = os.path.join('scripts', 'CA_' + financialYear + '.xlsx')
