    ...
    try
    {
        ...
        form.parse(req, async function (error, fields, file) 
        {
            ...
            else
            {
                ...
                {
                    let uploadFileToS3Bucket = await uniqueFunction.uploadFiles(encryptedData?.file, fileObject.uploadFile.originalFilename, clientUuid, documentFailedFolderPath)
                    if(uploadFileToS3Bucket && uploadFileToS3Bucket?.result)
                    {
                        ...
                        let sql = `UPDATE upload_doc_log_master SET failed_on = ?, failed_file_path = '${uploadFileToS3Bucket?.s3FilePath}' WHERE UPPER(file_name) = '${fileObject.uploadFile.originalFilename?.toUpperCase()}'`
