...
module.exports = require('express').Router().get('/',async(req,res) => 
{
    ...
    { 
        ...
        {
            await uniqueFunction.writeLogIntoFile({"creditNotePdfFilePaths" : []}, logFileName, "Get Credit Note Pdf List", getPath.getName('s3/creditNote/pdf'), 'green')
            res.status(200)
            return res.json({
                "status_code" : 200,
                "message"     : "success",
                "data"        : {"creditNotePdfFilePaths" : []},
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Credit Note Pdf List", getPath.getName('s3/creditNote/pdf'), 'red')
        console.log("e************", e)
        res.status(500)
        return res.json({
            "status_code" : 500,
...
async function getListObjectFromS3Bucket(clients, start, end, res, fileList)
{
    try
    {
        if(start < end)
        {
            ...
            s3.listObjectsV2(params,async(err, data) =>
            {
                if (err) 
                {
                    ...
                    await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, "Get Credit Note Pdf List: "+ s3FilePathListObject, getPath.getName('s3/creditNote/pdf'), 'red')
                    start++;
                    ...
                    content.forEach(file => {
                        if(file?.Key?.length > s3FilePathListObject?.length)
                        {
                            ...
                            if(path.extname(fileName)?.toLowerCase() != '.pdf')
                            {
                                ...
                                uniqueFunction.writeLogIntoFile("File type not accepted : " + path.extname(fileName), logFileName, "Get Credit Note Pdf List: "+ file?.Key, getPath.getName('s3/creditNote/pdf'), 'red')
                                console.log(params);
                                s3.getObject(params,(err, fileObj) =>
                                {
                                    ...
                                    {
                                        console.error('Error list:', err);
                                        uniqueFunction.writeLogIntoFile(err?.stack, logFileName, "Get Credit Note Pdf List: "+ file?.Key, getPath.getName('s3/creditNote/pdf'), 'red')
                                    } 
                                    else 
                                    {
        ...
        {
            await uniqueFunction.writeLogIntoFile({"creditNotePdfFilePaths" : fileList}, logFileName, "Get Credit Note Pdf List", getPath.getName('s3/creditNote/pdf'), 'green')
            
            let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Credit Note PDF', 'Bot File processing started, No. of files available :' + fileList.length, '', new Date())
            res.status(200)
            return res.json({
                "status_code" : 200,
                "message"     : "success",
                "data"        : {"creditNotePdfFilePaths" : fileList},
                "status_name" : getCode.getStatus(200)
            });
        }
    }
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Credit Note Pdf List => getListObjectFromS3Bucket", getPath.getName('s3/creditNote/pdf'), 'red')
        console.log(e)
        start++;
        getListObjectFromS3Bucket(clients, start, end, res,fileList)
    }
...
async function getListObjectFromLocal(clients, start, end, res, fileList)
{
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "Get Credit Note Pdf List => getListObjectFromLocal", getPath.getName('s3/creditNote/pdf'), 'red')
        console.log(e)
        start++;
        getListObjectFromLocal(clients, start, end, res,fileList)
    }
