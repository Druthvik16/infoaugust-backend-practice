...
module.exports = require('express').Router().get('/:partnerUuid?/:partnerLocationUuid?/:clientUuid?/:fromDate?/:toDate?/:dateType?', async (req, res) => {
    try {
        ...
        let getLastDocumentUploadOn = await db.getLastDocumentUploadDate(clientUuid, 'Ledger', partnerUuid, partnerLocationUuid, '', userTypeCode, loggedUserType)
        ...
        const getPartnerLedgers = await db.getPartnerLedgers(partnerUuid, partnerLocationUuid, clientUuid, fromDate, toDate, null, uploadedOn, userTypeCode, dateType, loggedUserType)
        
        res.status(200)
        return res.json({
            "status_code": 200,
            "message": "success",
            "data": { "partnerLedgers": getPartnerLedgers, 'totalMasterCount': getTotalCount[0].totalCount, lastUploadedOn },
