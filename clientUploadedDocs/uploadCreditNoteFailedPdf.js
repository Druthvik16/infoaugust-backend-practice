...
module.exports = require('express').Router().post('/',async(req,res) => 
{
    try
    {
        ...
        {
            await uniqueFunction.writeLogIntoFile("Provide all values", logFileName, req.body.fileName, getPath.getName('script/creditNote/pdf'), 'red')
            res.status(400)
            return res.json({
                "status_code" : 400,
        ...
        {
            await uniqueFunction.writeLogIntoFile("Encription key not found" + "________________" + JSON.stringify(encryptKey), logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'red')
            res.status(500)
            return res.json({
                "status_code" : 500,
        ...
        else
        {
            ...
            let s3SourceFilePathListObject = folderName + clientUuid + "/" + inputFolderPath + "/" + fileName            
    ...
    {
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'red')
        res.status(500)
        return res.json({
            "status_code" : 500,
    ...
    try
    {
        ...
        s3.getObject(params, async function(err, data) 
        {
            if (err) 
            {
                ...
                await uniqueFunction.writeLogIntoFile("Error copying object: " + JSON.stringify(err?.stack), logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'red')
                res.status(500)
                return res.json({
                    "status_code" : 500,
                    "message" : err?.stack,
            ...
            {   
                ...
                await uniqueFunction.writeLogIntoFile("Successfully get object for delete failed files : " + s3SourceFilePathListObject, logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'green')
                ...
                {                            
                    ...
                    s3.deleteObject(deleteParams, async function(err, data) 
                    {
                        if (err) 
                        {
                            ...
                            await uniqueFunction.writeLogIntoFile("Error deleting object: " + JSON.stringify(err?.stack), logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'red')
                            res.status(500)
                            return res.json({
                                "status_code" : 500,
                                "message" : err?.stack,
                        ...
                        else 
                        {
                            ...
                            {
                                await uniqueFunction.writeLogIntoFile("Failed Pdf File Moved To : " + uploadFileToS3Bucket?.s3FilePath, logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'blue')
                                res.status(200)
                                return res.json({
                                    "status_code" : 200,
                                    "message" : "success",
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Credit Note failed pdf not uploaded ", logFileName, fileName, getPath.getName('script/creditNote/pdf'), 'red')
                    res.status(500)
                    return res.json({
                        "status_code" : 500,
                        "message" : "Credit Note failed pdf not uploaded",
