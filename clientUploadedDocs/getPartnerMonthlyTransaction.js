...
module.exports = require('express').Router().get('/:partnerUuid?/:partnerLocationUuid?/:clientUuid?/:fromDate?/:toDate?/:dateType?', async (req, res) => {
    try {
        ...
        let getLastDocumentUploadOn = await db.getLastDocumentUploadDate(clientUuid, 'Transaction', partnerUuid, partnerLocationUuid, '', userTypeCode, loggedUserType)
        ...
        const getPartnerMonthlyTransactions = await db.getPartnerMonthlyTransactions(partnerUuid, partnerLocationUuid, clientUuid, fromDate, toDate, null, uploadedOn, userTypeCode, dateType, loggedUserType)
        res.status(200)
        return res.json({
            "status_code": 200,
            "message": "success",
            "data": { "partnerMonthlyTransactions": getPartnerMonthlyTransactions, 'totalMasterCount': getTotalCount[0].totalCount, lastUploadedOn },
