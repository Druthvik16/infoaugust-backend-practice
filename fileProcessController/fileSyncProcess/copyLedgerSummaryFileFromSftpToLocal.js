...
// };
let sourceFolder = 'LedgerSummary'
let destinationFolder = 'Input_Ledger_Raw_Sap_dump'
let clientId;
let clientUuid;
...
let apiName = '/api/fileProcessController/copyLedgerSummaryFileFromSftpToLocal'

let copyLedgerSummary = {}

copyLedgerSummary.getFileList = (readType) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            if(clients?.length == 0)
            {
                await uniqueFunction.writeLogIntoFile("Client List is empty", logFileName, "**************", getPath.getName('sftp/ledger'), 'red')
                return resolve(false)
            }
            ...
            documentCategoryId = documentCategories.find(category => category.name == 'Ledger')
            ...
            { 
                if(sftp.sftp)
                {
                    ...
                    try
                    {
                        await sftp.list(dir).then(async(list) => 
                        {
                            ...
                            list.forEach(async(file, index) => 
                            {
                                if(readType == 0)
                                {
                                    ...
                                    {  
                                        ...
                                        await uniqueFunction.writeLogIntoFile("Received Ledger Summary from Sftp server with size 0 " + "  ***************** dir :  " + JSON.stringify(file) , logFileName, file?.name, getPath.getName('sftp/ledger'), 'red')
                                    }
                                }
                                else
                                {
                            ...
                            if(files?.length > 0)
                            {
                                await uniqueFunction.writeLogIntoFile("Received Ledger Summary from Sftp server folder, Length is " + files?.length , logFileName, folderName, getPath.getName('sftp/ledger'), 'green')
    
                                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Ledger Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())
    
                                return resolve(getFileObject(files, 0, files.length, ''))
                            }
                            else
                            {
                                await uniqueFunction.writeLogIntoFile("Ledger Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, getPath.getName('sftp/ledger'), 'green')
                                await sftp?.end()
    
                                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Ledger Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())
    
                                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Ledger Summary', 'File Syncing completed', new Date())
                                return resolve(true)
                            }        
                        })
                        .catch(async(err) => 
                        {
                            ...
                            await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, folderName, getPath.getName('sftp/ledger'), 'red')
                            await sftp?.end()
                            return resolve(false)
                        })
                    }
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Sftp connection not created", logFileName, "*******************", getPath.getName('sftp/ledger'), 'red')
                    return resolve(false)
                }              
            }).catch(async (err) => {
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, "*******************", getPath.getName('sftp/ledger'), 'red')
                let result = await connectSFTP(1)
                if(!result)
                {
                    return resolve(false);
                }
                return copyLedgerSummary.getFileList(readType)
            })
        }
        catch(e)
        {
    
            await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "*******************", getPath.getName('sftp/ledger'), 'red')
            console.log(e)
            await sftp?.end()
            return resolve(false)
        }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            console.log("Coping process started for ledger file " + start + " from " + end)
            let file = files[start]
            await uniqueFunction.writeLogIntoFile("Coping process started for ledger file " + (start + 1) + " from " + end, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'Purple')
            ...
            let uniqueCheckName = await uniqueFunction.unquieName(identifierName, ['file_name'],{  "file_name" : file.fileName }, id, 0)
                try
                {
                    if(uniqueCheckName != 0)
                    {
                        ...
                        await uniqueFunction.writeLogIntoFile(`The file name already exists. Therefore, this file is being renamed to '${newFileName}'.`, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'Tomato') 
                    }
                ...
                {
                    await uniqueFunction.writeLogIntoFile(sftpError?.stack, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
        ...
        {
            console.log("Ledger Summary File Copied")
            await uniqueFunction.writeLogIntoFile("Ledger Summary File Copied", logFileName, files[start-1].fileName, getPath.getName('sftp/ledger'), 'green')
            // await sftp?.end()
            console.log("Ledger Summary Process Completed")
            await sftp?.end()
    ...
    {
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, files[start].fileName, getPath.getName('sftp/ledger'), 'red')
        start++
        return  getFileObject(files, start, end, res)
    }
}
...
async function getFileFromSftp(file, start, end, res, uniqueCheckName)
{
    try
    {
        ...
        if(encryptedData?.result)
        {
            ...
            if(uploadFiles && uploadFiles.result == true)
            {
                ...
                await uniqueFunction.writeLogIntoFile("File uploaded and database log updatd", logFileName, file.fileName, getPath.getName('sftp/ledger'), 'green')
                console.log("remane1")
            ...
            {
                await uniqueFunction.writeLogIntoFile("File not uploaded", logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
                return  true;
            } 
        }
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile("File encryption error : " + JSON.stringify(encryptedData?.error), logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
            return  true;
        } 
    }
    catch(e)
    {
        ...
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
        let result = await connectSFTP(1)
        if(!result)
        {
            return false;
...
const connectSFTP = async (count) => 
{
    ...
    catch (error) 
    {
        ...
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile(error?.stack, logFileName, "*********", getPath.getName('sftp/ledger'), 'red')
    ...
    return new Promise(async(resolve, reject) => 
        {
            ...
            try
            {
                ...
                await uniqueFunction.writeLogIntoFile(`*************File Renamed (${file.fileName} to ${newFileName}) *******`, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'green')
                return resolve(true)
            }
            catch(rnameErr)
            {
                await uniqueFunction.writeLogIntoFile(rnameErr?.stack, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
        ...
        {
            await uniqueFunction.writeLogIntoFile("Sftp connection lost file rename failed", logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
            return resolve(true)
        }
    })
}
...
module.exports = copyLedgerSummary
