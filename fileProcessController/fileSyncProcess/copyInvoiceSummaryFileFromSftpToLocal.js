...
// };
let sourceFolder = 'InvoiceSummary'
let destinationFolder = 'Input_Invoice_Summary_Raw_Sap_dump'
let clientId;
let clientUuid;
...
let apiName = '/api/fileProcessController/copyInvoiceSummaryFileFromSftpToLocal'

let copyInvoiceSummary = {}

copyInvoiceSummary.getFileList = (readType) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (clients?.length == 0) {
                await uniqueFunction.writeLogIntoFile("Client List is empty", logFileName, "**************", getPath.getName('sftp/invoice/summary'), 'red')
                return resolve(false)
            }
                ...
                if (sftp.sftp) {
                    ...
                    try {
                        await sftp.list(dir).then(async (list) => {
                            ...
                            list.forEach(async (file, index) => {
                                if (readType == 0) {
                                    ...
                                    if (file && file?.type == '-' && fileEnd != 'read' && documentAttachments?.length > 0 && documentCategories?.length > 0 && file?.size > 0) {
                                        ...
                                        await uniqueFunction.writeLogIntoFile("Received Invoice summary from Sftp server with size 0 " + "  ***************** dir :  " + JSON.stringify(file), logFileName, file?.name, getPath.getName('sftp/invoice/summary'), 'red')
                                    }
                                }
                                else {
                                    let fileEnd = (file?.name.split('_').pop())?.split('.')[0]
                            ...
                            if (files?.length > 0) {
                                await uniqueFunction.writeLogIntoFile("Received Invoice summary from Sftp server folder, Length is " + files?.length, logFileName, folderName, getPath.getName('sftp/invoice/summary'), 'green')

                                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Invoice Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                                return resolve(getFileObject(files, 0, files.length, ''))
                            }
                            else {
                                await uniqueFunction.writeLogIntoFile("Invoice Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, getPath.getName('sftp/invoice/summary'), 'green')
                                await sftp?.end()

                                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Invoice Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Invoice Summary', 'File Syncing completed', new Date())
                                return resolve(true)
                            }
                        })
                            .catch(async (err) => {
                                ...
                                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, folderName, getPath.getName('sftp/invoice/summary'), 'red')
                                await sftp?.end()
                                return resolve(false)
                            })
                    }
                    ...
                    await uniqueFunction.writeLogIntoFile("Sftp connection not created", logFileName, "*******************", getPath.getName('sftp/invoice/summary'), 'red')
                    return resolve(false)
                }
            }).catch(async (err) => {
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, "*******************", getPath.getName('sftp/invoice/summary'), 'red')
                let result = await connectSFTP(1)
                if (!result) {
                    return resolve(false);
                }
                return copyInvoiceSummary.getFileList(readType)
            })
        }
        catch (e) {
            await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "*******************", getPath.getName('sftp/invoice/summary'), 'red')
            console.log(e)
            await sftp?.end()
            return resolve(false)
        }
...
async function getFileObject(files, start, end, res) {
    try {
        ...
        if (start < end) {
            console.log("Coping process started for invoice summary file " + start + " from " + end)
            let file = files[start]
            await uniqueFunction.writeLogIntoFile("Coping process started for invoice summary file " + (start + 1) + " from " + end, logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'Purple')
            ...
            try {
                if (uniqueCheckName != 0) {
                    ...
                    await uniqueFunction.writeLogIntoFile(`The file name already exists. Therefore, this file is being renamed to '${newFileName}'.`, logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'Tomato')
                }
                ...
                await uniqueFunction.writeLogIntoFile(sftpError?.stack, logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'red')
            ...
            console.log("Invoice Summary File Copied")
            await uniqueFunction.writeLogIntoFile("Invoice Summary File Copied", logFileName, files[start - 1].fileName, getPath.getName('sftp/invoice/summary'), 'green')
            console.log("Invoice Summary Process Completed")
            await sftp?.end()
        ...
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, files[start].fileName, getPath.getName('sftp/invoice/summary'), 'red')
        start++
        return getFileObject(files, start, end, res)
    }
}
    ...
    try {
        ...
        if (encryptedData?.result) {
            ...
            if (uploadFiles && uploadFiles.result == true) {
                ...
                await uniqueFunction.writeLogIntoFile("File uploaded and database log updatd", logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'green')
                console.log("remane1")
                ...
                await uniqueFunction.writeLogIntoFile("File not uploaded", logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'red')
                return true;
            }
        }
        else {
            console.log("file encryption error")
            await uniqueFunction.writeLogIntoFile("File encryption error : " + JSON.stringify(encryptedData?.error), logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'red')
            return true;
        }
    }
    catch (e) {
        ...
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'red')
        let result = await connectSFTP(1)
        if (!result) {
            return false;
        }
...
const connectSFTP = async (count) => {
    try {
        ...
        if (count >= 10) {
            ...
            await uniqueFunction.writeLogIntoFile(error?.stack, logFileName, "*********", getPath.getName('sftp/invoice/summary'), 'red')
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (sftp?.sftp) {
            ...
            try {
                ...
                await uniqueFunction.writeLogIntoFile(`*************File Renamed (${file.fileName} to ${newFileName}) *******`, logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'green')
                return resolve(true)
            }
            catch (rnameErr) {
                await uniqueFunction.writeLogIntoFile(rnameErr?.stack, logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'red')
                console.log("Error renaming", rnameErr)
                return resolve(true)
            }
        }
        else {
            await uniqueFunction.writeLogIntoFile("Sftp connection lost file rename failed", logFileName, file.fileName, getPath.getName('sftp/invoice/summary'), 'red')
            return resolve(true)
        }
    })
}
...
module.exports = copyInvoiceSummary
