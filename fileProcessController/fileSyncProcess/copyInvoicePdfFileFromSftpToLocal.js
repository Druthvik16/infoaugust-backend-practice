...
let apiName = '/api/fileProcessController/copyInvoicePdfFileFromSftpToLocal'

let copyInvoicePdf = {}

copyInvoicePdf.getFileList = (readType) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            { 
                ...
                return copyInvoicePdf.getFileList(readType)
            })
        }
        catch(e)
        {
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            console.log("Coping process started for invoice pdf file " + start + " from " + end)
...
async function getFileFromSftp(file, start, end, res, uniqueCheckName)
{
    ...
    catch(e)
    {
        ...
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, file.fileName, getPath.getName('sftp/invoice/pdf'), 'red')
        let result = await connectSFTP(1)
        if(!result)
        {
            return false;
...
const connectSFTP = async (count) => 
{
    ...
    catch (error) 
    {
        ...
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile(error?.stack, logFileName, "*********", getPath.getName('sftp/invoice/pdf'), 'red')
