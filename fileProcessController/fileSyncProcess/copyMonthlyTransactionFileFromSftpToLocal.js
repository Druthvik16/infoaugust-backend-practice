...
// };
let sourceFolder = 'CTransaction'
let destinationFolder = 'Input_Monthly_Transactions_Raw_Sap_dump'
let clientId;
let clientUuid;
...
let apiName = '/api/fileProcessController/copyMonthlyTransactionSummaryFileFromSftpToLocal'

let copyMonthlyTransactionSummary = {}

copyMonthlyTransactionSummary.getFileList = (readType) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            if(clients?.length == 0)
            {
                await uniqueFunction.writeLogIntoFile("Client List is empty", logFileName, "**************", getPath.getName('sftp/monthlyTransaction'), 'red')
                return resolve(false)
            }
            ...
            documentCategoryId = documentCategories.find(category => category.name == 'Transaction')
            ...
            { 
                if(sftp.sftp)
                {
                    ...
                    try
                    {
                        await sftp.list(dir).then(async(list) => 
                        {
                            ...
                            list.forEach(async(file, index) => 
                            {
                                if(readType == 0)
                                {
                                    ...
                                    {  
                                        ...
                                        await uniqueFunction.writeLogIntoFile("Received Monthly Transaction Summary from Sftp server with size 0 " + "  ***************** dir :  " + JSON.stringify(file) , logFileName, file?.name, getPath.getName('sftp/monthlyTransaction'), 'red')
                                    }
                                }
                                else
                                {                                        
                            ...
                            if(files?.length > 0)
                            {
                                await uniqueFunction.writeLogIntoFile("Received Monthly Transaction Summary from Sftp server folder, Length is " + files?.length , logFileName, folderName, getPath.getName('sftp/monthlyTransaction'), 'green')
    
                                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Monthly Transaction Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())
    
                                return resolve(getFileObject(files, 0, files.length, ''))
                            }
                            else
                            {
                                await uniqueFunction.writeLogIntoFile("Monthly Transaction Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, getPath.getName('sftp/monthlyTransaction'), 'red')
                                await sftp?.end()
    
                                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Monthly Transaction Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())
    
                                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Monthly Transaction Summary', 'File Syncing completed', new Date())
                                return resolve(true)
                            }        
                        })
                        .catch(async(err) => 
                        {
                            ...
                            await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, folderName, getPath.getName('sftp/monthlyTransaction'), 'red')
                            await sftp?.end()
                            return resolve(false)
                        })
                    }
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Sftp connection not created", logFileName, "*******************", getPath.getName('sftp/monthlyTransaction'), 'red')
                    return resolve(false)
                }              
            }).catch(async (err) => {
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, "*******************", getPath.getName('sftp/monthlyTransaction'), 'red')
                let result = await connectSFTP(1)
                if(!result)
                {
                    return resolve(false);
                }
                return copyMonthlyTransactionSummary.getFileList(readType)
            })
        }
        catch(e)
        {
    
            await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "*******************", getPath.getName('sftp/monthlyTransaction'), 'red')
            console.log(e)
            await sftp?.end()
            return resolve(false)
        }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            console.log("Coping process started for monthly transaction file " + start + " from " + end)
            let file = files[start]
            await uniqueFunction.writeLogIntoFile("Coping process started for monthly transaction file " + (start + 1) + " from " + end, logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'Purple')
            ...
            let uniqueCheckName = await uniqueFunction.unquieName(identifierName, ['file_name'],{  "file_name" : file.fileName }, id, 0)
                ...
                try
                {
                    if(uniqueCheckName != 0)
                    {
                        ...
                        await uniqueFunction.writeLogIntoFile(`The file name already exists. Therefore, this file is being renamed to '${newFileName}'.`, logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'Tomato') 
                    }
                ...
                {
                    await uniqueFunction.writeLogIntoFile(sftpError?.stack, logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
        ...
        {
            console.log("Monthly Transaction Summary File Copied")
            await uniqueFunction.writeLogIntoFile("Monthly Transaction Summary File Copied", logFileName, files[start-1].fileName, getPath.getName('sftp/monthlyTransaction'), 'green')
            await sftp?.end()

            let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Monthly Transaction Summary', 'File Syncing completed', new Date())
            console.log("Monthly Transaction Summary Process Completed")
            return true
        }
    }
    catch(e)
    {
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, files[start].fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
        start++
        return  getFileObject(files, start, end, res)
    }
}
...
async function getFileFromSftp(file, start, end, res, uniqueCheckName)
{
    try
    {
        ...
        if(encryptedData?.result)
        {
            ...
            if(uploadFiles && uploadFiles.result == true)
            {
                ...
                await uniqueFunction.writeLogIntoFile("File uploaded and database log updatd", logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'green')
                console.log("remane1")
            ...
            {
                await uniqueFunction.writeLogIntoFile("File not uploaded", logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
                return  true;
            } 
        }
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile("File encryption error : " + JSON.stringify(encryptedData?.error), logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
            return  true;
        } 
    }
    catch(e)
    {
        ...
        console.log(e)
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
        let result = await connectSFTP(1)
        if(!result)
        {
            return false;
...
const connectSFTP = async (count) => 
{
    ...
    catch (error) 
    {
        ...
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile(error?.stack, logFileName, "*********", getPath.getName('sftp/monthlyTransaction'), 'red')
    ...
    {       
        ...
        if(sftp?.sftp)
        {
            ...
            try
            {
                ...
                await uniqueFunction.writeLogIntoFile(`*************File Renamed (${file.fileName} to ${newFileName}) *******`, logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'green')
                return resolve(true)
            }
            catch(rnameErr)
            {
                await uniqueFunction.writeLogIntoFile(rnameErr?.stack, logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
        ...
        {
            await uniqueFunction.writeLogIntoFile("Sftp connection lost file rename failed", logFileName, file.fileName, getPath.getName('sftp/monthlyTransaction'), 'red')
            return resolve(true)
        }
    })
}

module.exports = copyMonthlyTransactionSummary
