...
module.exports = require('express').Router().get('/:action',async(req,res) => 
{
    try
    {
        let getUnallocatedUsers;
        let usersList = []
        let action;
        // getUnallocatedUsers
        action = req.params.action
        getUnallocatedUsers = await db.getUnallocatedUsers(action)
        if(getUnallocatedUsers.length == 0)
        {
            res.status(200)
            return res.json({
                "status_code" : 200,
                "message"     : "success",
                "data"        : {"unallocatedUsers" : []},
        ...
        {
            usersList = []
            getUnallocatedUsers.forEach((element) => 
            {
            ...
            return res.json({
                "status_code" : 200,
                "message"     : "success",
                "data"        : {"unallocatedUsers" : usersList},
                "status_name" : getCode.getStatus(200)
            });
        }
    }
