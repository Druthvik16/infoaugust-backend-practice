...
const sourceFolder = 'spsndocs/CAReportSummary'
const destinationFolder = 'Input_Adjustment_Report_Summary_Sap_Dump'
const logFileNamePrefix = "adjustmentReportSummaryLogFile-"
const apiName = '/api/spsnFileProcessController/copyAdjustmentReportSummaryFileFromSftpToLocal'
const commonSyncFunction = require('./commonSyncFunction')
const logFilePath = getPath.getName('spsn/sftp/adjustmentReport/summary')
let logFileName = ''
const copyAdjustmentReportSummary = {}
let config = ''
const sftp = new Client();
let clientID;

copyAdjustmentReportSummary.getFileList = (readType, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let documentCategoryId = documentCategories.find(category => category.name == 'Adjustment Report')
            ...
            try
            {
                ...
                list.forEach(async(file, index) => 
                {
                    if(readType == 0)
                    {
                        ...
                        {  
                            ...
                            await uniqueFunction.writeLogIntoFile("Received Adjustment Report summary from Sftp server with size 0 " + "  *****************   " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                        }
                    }
                    else
                    {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received Adjustment Report summary Sftp server folder Length is " + files?.length , logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Adjustment Report Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    return resolve(getFileObject(files, 0, files.length, ''))
                }
                else
                {
                    await uniqueFunction.writeLogIntoFile("Adjustment Report Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Adjustment Report Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Adjustment Report Summary', 'File Syncing completed', new Date())
                    await sftp?.end()
                    return resolve(true)
                }    
            }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        if(start < end)
        {
            console.log("Coping process started for adjustment report summary file " + start + " from " + end)
            ...
            await uniqueFunction.writeLogIntoFile("Coping process started for adjustment report summary file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
        ...
        {
            console.log("Adjustment Report Summary File Copied")
            await uniqueFunction.writeLogIntoFile("Adjustment Report Summary File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
            console.log("Adjustment Report Summary Process Completed")
...
async function mailFiles(file, fileData)
{
    return  new Promise(async(resolve, reject) => 
    {
        ...
        let dataToSend = {
            ...
            "subject": "Adjustment Report Summary Raw File",
            "text": "<div>Hello,</div><br/><br/><div>Please find attachment of Adjustment Report Summary</div>",
            "rawFiles" : attachment
        }
...
module.exports = copyAdjustmentReportSummary
