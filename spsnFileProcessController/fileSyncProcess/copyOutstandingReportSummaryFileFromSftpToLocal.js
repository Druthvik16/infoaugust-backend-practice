...
const sourceFolder = 'spsndocs/OSReportSummary'
const destinationFolder = 'Input_Outstanding_Report_Summary_Sap_Dump'
const logFileNamePrefix = "outstandingReportSummaryLogFile-"
const apiName = '/api/spsnFileProcessController/copyOutstandingReportSummaryFileFromSftpToLocal'
const commonSyncFunction = require('./commonSyncFunction')
const logFilePath = getPath.getName('spsn/sftp/outstandingReport/summary')
let logFileName = ''
const copyOutstandingReportSummary = {}
let config = ''
const sftp = new Client();
let clientID;

copyOutstandingReportSummary.getFileList = (readType, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let documentCategoryId = documentCategories.find(category => category.name == 'Outstanding Report')
            ...
            try
            {
                ...
                list.forEach(async(file, index) => 
                {
                    if(readType == 0)
                    {
                        ...
                        {  
                            ...
                            await uniqueFunction.writeLogIntoFile("Received Outstanding Report summary from Sftp server with size 0 " + "  *****************   " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                        }
                    }
                    else
                    {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received Outstanding Report summary Sftp server folder Length is " + files?.length , logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Outstanding Report Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    return resolve(getFileObject(files, 0, files.length, ''))
                }
                else
                {
                    await uniqueFunction.writeLogIntoFile("Outstanding Report Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Outstanding Report Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Outstanding Report Summary', 'File Syncing completed', new Date())
                    await sftp?.end()
                    return resolve(true)
                }    
            }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        if(start < end)
        {
            console.log("Coping process started for outstanding report summary file " + start + " from " + end)
            let file = files[start]
            let interuptProcess = await db.getInteruptProcess(file?.clientId);
            if(interuptProcess[0].isWorking == 0)
            {
                await sftp?.end()
                return false;
            }
            await uniqueFunction.writeLogIntoFile("Coping process started for outstanding report summary file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
            let identifierName = 'client_spsn_upload_doc_log_master'            
            let id = 0
        ...
        {
            console.log("Outstanding Report Summary File Copied")
            await uniqueFunction.writeLogIntoFile("Outstanding Report Summary File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
            console.log("Outstanding Report Summary Process Completed")
...
async function getFileFromSftp(file, start, end, res, uniqueCheckName)
{
    ...
    {        
        ...
        if(encryptedData?.result)
        {
            ...
            // let uploadFiles = await uniqueFunction.singleFileUpload(sourceFileContents, getPath.getName('documentFolders') + '/spsnModule/client', file.fileName, (file.clientUuid + '/' + file.destinationFolder))
            let uploadFiles = await uniqueFunction.singleFileUpload(sourceFileContents, getPath.getName('documentFolders') + '/'+ uniqueFunction.spsnModule + 'client', file.fileName, (file.clientUuid + '/' + file.destinationFolder))
...
async function mailFiles(file, fileData)
{
    return  new Promise(async(resolve, reject) => 
    {
        ...
        let dataToSend = {
            ...
            "subject": "Outstanding Report Summary Raw File",
            "text": "<div>Hello,</div><br/><br/><div>Please find attachment of Outstanding Report Summary</div>",
            "rawFiles" : attachment
        }
...
module.exports = copyOutstandingReportSummary
