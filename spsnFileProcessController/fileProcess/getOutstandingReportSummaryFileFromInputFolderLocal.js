...
let sourceFolder = 'Input_Outstanding_Report_Summary_Sap_Dump'
let clients;
let docPath = require('../../DOC_FOLDER_PATH/docPath')
let getPath = new docPath()
let logFileName = "outstandingReportSummaryLogFile-"
let apiName = '/api/spsnFileProcessController/getOutstandingReportSummaryFileFromInputFolderLocal'
let processOutstandingReportSummary = require('../scriptProcessing/processOutstandingReportSummaryLocal')
const logFilePath = getPath.getName('spsn/s3/outstandingReport/summary')

let getOutstandingReportSummary = {}
let clientID;

getOutstandingReportSummary.getFileList = (client) => 
{
...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            // let localFilePathListObject = getPath.getName('documentFolders') + '/spsnModule/client' + "/"  + clientUuid + "/" + sourceFolder + "/"
            let localFilePathListObject = getPath.getName('documentFolders') + '/'+ uniqueFunction.spsnModule + 'client' + "/"  + clientUuid + "/" + sourceFolder + "/"
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Outstanding Report Summary', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Outstanding Report Summary', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Outstanding Report summary local folder is empty", logFileName, "*******************", logFilePath, 'green')
                start++;
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        { 
            ...
            try
            {
                console.log("Getting outstanding report summary file for processing , file :" + (start + 1) + " from " + end)
                await uniqueFunction.writeLogIntoFile("Getting outstanding report summary file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('spsn/sftp/outstandingReport/summary'), 'Purple') 
                ...
                {
                    file['encryptionKey'] = encryptKey[0].encryption_key
                    file['encryptionIV'] = encryptKey[0].encryption_iv
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling outstanding report summary script", logFileName, file?.task.Key, logFilePath, 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling outstanding report summary script", file.task.Key)
                        let isFileSaved = await sendToOSScript(file)
                        if(isFileSaved)
                        {
                            start++;
                            return getFileObject(files, start, end, masterStart, masterEnd, res);
...
async function sendToOSScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let processFile = await processOutstandingReportSummary.getFileList(params.uploadOSFile, params.inputFolderPath, params.encryptionIV, params.encryptionKey, params.client)
...
module.exports = getOutstandingReportSummary
