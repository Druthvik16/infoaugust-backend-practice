...
let sourceFolder = 'Input_Adjustment_Report_Summary_Sap_Dump'
let clients;
let docPath = require('../../DOC_FOLDER_PATH/docPath')
let getPath = new docPath()
let logFileName = "adjustmentReportSummaryLogFile-"
let apiName = '/api/spsnFileProcessController/getAdjustmentReportSummaryFileFromInputFolderLocal'
let processAdjustmentReportSummary = require('../scriptProcessing/processAdjustmentReportSummaryLocal')
const logFilePath = getPath.getName('spsn/s3/adjustmentReport/summary')

let getAdjustmentReportSummary = {}
let clientID;

getAdjustmentReportSummary.getFileList = (client) => 
{
...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Adjustment Report Summary', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Adjustment Report Summary', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Adjustment Report summary local folder is empty", logFileName, "*******************", logFilePath, 'green')
                start++;
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        { 
            ...
            try
            {
                console.log("Getting adjustment report summary file for processing , file :" + (start + 1) + " from " + end)
                await uniqueFunction.writeLogIntoFile("Getting adjustment report summary file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('spsn/sftp/adjustmentReport/summary'), 'Purple') 
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling adjustment report summary script", logFileName, file?.task.Key, logFilePath, 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling adjustment report summary script", file.task.Key)
                        let isFileSaved = await sendToCAScript(file)
                        if(isFileSaved)
                        {
                            start++;
                            return getFileObject(files, start, end, masterStart, masterEnd, res);
...
async function sendToCAScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let processFile = await processAdjustmentReportSummary.getFileList(params.uploadCAFile, params.inputFolderPath, params.encryptionIV, params.encryptionKey, params.client)
...
module.exports = getAdjustmentReportSummary
