...
router.post('/', async (req, res) => {
    const { uuid, userId, mobile, partnerLocationDetailUuids } = req.body;
    try {
        const id = 0;
        ...
        if (!uuid || !name?.trim() || !partnerLocationDetailUuids || partnerLocationDetailUuids?.length == 0 || !email || !mobile) {
            res.status(400)
            return res.json({
                "status_code": 400,
        ...
        let uniqueCheckEmailInPartnerSecondary = await uniqueFunction.unquieName('secondary_partner', ['email'], { "email": email }, id, uuid)
        ...
        let uniqueCheckMobileInPartnerSecondary = await uniqueFunction.unquieName('secondary_partner', ['mobile'], { "mobile": mobile }, id, uuid)
        ...
        const secondaryPartnerResult = await db.updateSecondaryPartner(uuid, name, email, mobile, userId);

        const secondaryPartner = await commonDb.getIdByUUId('secondary_partner', uuid)

        const secondaryPartnerId = secondaryPartner[0].id;

        const deleteLocations = await db.deleteSecondaryPartnerMappedLocations(secondaryPartnerId)
