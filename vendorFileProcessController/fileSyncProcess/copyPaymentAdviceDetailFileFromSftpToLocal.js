...
const sourceFolder = 'vendorDocs/PaymentAdvice/Detail'
const destinationFolder = 'Input_Payment_Advice_Detail_Raw_Sap_dump'
const logFileNamePrefix = "paymentAdviceDetailLogFile-"
const apiName = '/api/vendorFileProcessController/copyPaymentAdviceDetailFileFromSftpToLocal'
const commonSyncFunction = require('./commonSyncFunction')
const logFilePath = getPath.getName('vendor/sftp/paymentAdviceDetail')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let logFileName = ''
const copyPaymentAdviceDetail = {}
let config = ''
let clientID;
const sftp = new Client();

copyPaymentAdviceDetail.getFileList = (readType, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            try
            {
                ...
                list.forEach(async(file, index) => 
                {
                    if(readType == 0)
                    {
                        ...
                        {  
                            ...
                            await uniqueFunction.writeLogIntoFile("Received payment advice detail from Sftp server with size 0 " + "  *****************   " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                        }
                    }
                    else
                    {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received payment advice detail Sftp server folder Length is " + files?.length , logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Payment Advice Detail', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    return resolve(getFileObject(files, 0, files.length, ''))
                }
                else
                {
                    await uniqueFunction.writeLogIntoFile("Payment Advice Detail Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Payment Advice Detail', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Payment Advice Detail', 'File Syncing completed', new Date())
                    await sftp?.end()
                    return resolve(true)
                }    
            }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            console.log("Coping process started for payment advice detail file " + start + " from " + end)
            let file = files[start]
            await uniqueFunction.writeLogIntoFile("Coping process started for payment advice detail file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
        ...
        {
            console.log("Payment Advice Detail File Copied")
            await uniqueFunction.writeLogIntoFile("Payment Advice Detail File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
            console.log("Payment Advice Detail Process Completed")
...
async function mailFiles(file, fileData)
{
    return  new Promise(async(resolve, reject) => 
    {
        ...
        let dataToSend = {
            ...
            "subject": "Payment Advice Detail Raw File",
            "text": "<div>Hello,</div><br/><br/><div>Please find attachment of Payment Advice Detail</div>",
            "rawFiles" : attachment
        }
...
module.exports = copyPaymentAdviceDetail
