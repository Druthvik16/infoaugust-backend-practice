...
const sourceFolder = 'vendorDocs/Form16ASummary'
const destinationFolder = 'Input_form16_Summary_Raw_Sap_dump'
const logFileNamePrefix = "form16SummaryLogFile-"
const apiName = '/api/vendorFileProcessController/copyForm16SummaryFileFromSftpToLocal'
const commonSyncFunction = require('./commonSyncFunction')
const logFilePath = getPath.getName('vendor/sftp/form16/summary')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let logFileName = ''
const copyForm16Summary = {}
let config = ''
const sftp = new Client();
let clientID;

copyForm16Summary.getFileList = (readType, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            try
            {
                ...
                list.forEach(async(file, index) => 
                {
                    if(readType == 0)
                    {
                        ...
                        {  
                            ...
                            await uniqueFunction.writeLogIntoFile("Received Form16A summary from Sftp server with size 0 " + "  *****************   " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                        }
                    }
                    else
                    {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received Form16A summary Sftp server folder Length is " + files?.length , logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Form16A Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    return resolve(getFileObject(files, 0, files.length, ''))
                }
                else
                {
                    await uniqueFunction.writeLogIntoFile("Form16A Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Form16A Summary', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Form16A Summary', 'File Syncing completed', new Date())
                    await sftp?.end()
                    return resolve(true)
                }    
            }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            console.log("Coping process started for form 16 summary file " + start + " from " + end)
            let file = files[start]
            await uniqueFunction.writeLogIntoFile("Coping process started for form 16 summary file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
        ...
        {
            console.log("Form16A Summary File Copied")
            await uniqueFunction.writeLogIntoFile("Form16A Summary File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
            console.log("Form16A Summary Process Completed")
...
const connectSFTP = async (count) => 
{
    try 
    {
        ...
        await uniqueFunction.writeLogIntoFile(JSON.stringify(config), logFileName, "*******************", logFilePath, 'red')
        if (!sftp.sftp) 
        {
...
async function mailFiles(file, fileData)
{
    return  new Promise(async(resolve, reject) => 
    {
        ...
        let dataToSend = {
            ...
            "subject": "Form16A Summary Raw File",
            "text": "<div>Hello,</div><br/><br/><div>Please find attachment of Form16A Summary</div>",
            "rawFiles" : attachment
        }
...
module.exports = copyForm16Summary
