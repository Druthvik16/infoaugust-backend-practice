...
let config = ''
let sourceFolder = 'vendorDocs/LedgerSummary'
let destinationFolder = 'Input_Ledger_Raw_Sap_dump'
let docPath = require('../../DOC_FOLDER_PATH/docPath')
let getPath = new docPath()
let logFileName = "ledgerSummaryLogFile-"
let apiName = '/api/vendorFileProcessController/copyLedgerSummaryFileFromSftpToLocal'
const logFilePath = getPath.getName('vendor/sftp/ledger');
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let clientID;

let copyLedgerSummary = {}

copyLedgerSummary.getFileList = (readType, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let documentAttachments = await db.getDocumentAttachemnts();
            ...
            let documentCategoryId = documentCategories.find(category => category.name == 'Ledger')
            ...
            try
            {
                ...
                list.forEach(async(file, index) => 
                {
                    if(readType == 0)
                    {
                        ...
                        {  
                            ...
                            await uniqueFunction.writeLogIntoFile("Received Ledger from Sftp server with size 0 " + "  ***************** dir :  " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                        }
                    }
                    else
                    {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received Ledger from Sftp server folder, Length is " + files?.length , logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Ledger', 'File syncing started, No. of files available :' + files.length, '', new Date())
                    return resolve(getFileObject(files, 0, files.length, ''))
                }
                else
                {
                    await uniqueFunction.writeLogIntoFile("Ledger Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, logFilePath, 'green')

                    let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('SFTP Sync To Local', 'Ledger', 'File syncing started, No. of files available :' + files.length, '', new Date())

                    let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetail('SFTP Sync To Local', 'Ledger', 'File Syncing completed', new Date())
                    await sftp?.end()
                    return resolve(true)
                }   
            }
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            await uniqueFunction.writeLogIntoFile("Coping process started for ledger file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
        ...
        {
            console.log("Ledger File Copied")
            await uniqueFunction.writeLogIntoFile("Ledger File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
            // await sftp?.end()
            console.log("Ledger Process Completed")
            await sftp?.end()
