...
const sourceFolder = 'vendorDocs/CreditNoteSummary'
const destinationFolder = 'Input_Summary_Raw_Sap_dump'
const logFileNamePrefix = "creditNoteSummaryLogFile-"
const apiName = '/api/vendorFileProcessController/copyCreditNoteSummaryFileFromSftpToLocal'
const commonSyncFunction = require('./commonSyncFunction')
const logFilePath = getPath.getName('vendor/sftp/creditNote/summary')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let logFileName = ''
const copyCreditNoteSummary = {}
let config = ''
const sftp = new Client();
let clientID;

copyCreditNoteSummary.getFileList = (readType, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let documentCategoryId = documentCategories.find(category => category.name == 'Credit Note')
            ...
            try
            {
                ...
                list.forEach(async(file, index) => 
                {
                    if(readType == 0)
                    {
                        ...
                        {  
                            ...
                            await uniqueFunction.writeLogIntoFile("Received Credit Note summary from Sftp server with size 0 " + "  *****************   " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                        }
                    }
                    else
                    {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received Credit Note summary Sftp server folder Length is " + files?.length , logFileName, folderName, logFilePath, 'green')
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Credit Note Summary Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, logFilePath, 'green')
...
async function getFileObject(files, start, end, res)
{
    try
    {
        if(start < end)
        {
            ...
            await uniqueFunction.writeLogIntoFile("Coping process started for credit note summary file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
        ...
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile("Credit Note Summary File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
