...
let sourceFolders = ['vendorDocs/CreditNotePDF', 'vendorDocs/DebitNotePDF']
...
const logFilePath = getPath.getName('vendor/sftp/creditNote/pdf')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let readType = ''

let copyCreditNotePdf = {}

copyCreditNotePdf.getFileList = (readType1, client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        ...
        try
        {
            ...
            console.log(JSON.parse(client.algorithms))
            config = {
                host: client.host,
                port: client.port,
            ...
            try
            {
                ...
                let files = await getSourceFolder(sourceFolders, 0, sourceFolders?.length, [])
                ...
                //             await uniqueFunction.writeLogIntoFile("Received Credit Note Pdf from Sftp server with size 0 " + "  *****************   " + JSON.stringify(file) , logFileName, file?.name, logFilePath, 'red')
                //         }
                //     }
                //     else
                //     {
                ...
                if(files?.length > 0)
                {
                    await uniqueFunction.writeLogIntoFile("Received Credit Note Pdf Sftp server folder Length is " + files?.length , logFileName, '', logFilePath, 'green')
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Credit Note Pdf Sftp server folder is empty", logFileName, '', logFilePath, 'green')
            ...
            {
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, '', logFilePath, 'red')
...
async function getSourceFolder(sourceFolders, start, end, files)
{ 
    try
    {
    ...
    {
        return getSourceFolder(sourceFolders, start, end, files)
    }
    ...
    if(start < end)
    {
        ...
        try
        {
            console.log("0************files*****************", files?.length)
            await uniqueFunction.writeLogIntoFile("listing process started for credit note pdf of folders " + folderName +" file " + (start + 1) + " from " + end, logFileName, " ****** ", logFilePath, 'Purple')
            let list = await sftp.list(dir)
            console.log("list*****************", list?.length, folderName)
            await uniqueFunction.writeLogIntoFile("listing " + dir + " totalFiles " + list?.length, logFileName, " ****** ", logFilePath, 'Purple')
            list.forEach(async(file, index) => 
            {
                if(readType == 0)
                {
            ...
            console.log("1************files*****************", files?.length)
            return getSourceFolder(sourceFolders, start + 1, end, files) 
        }
        catch (err)
        {
...
async function getFileObject(files, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            await uniqueFunction.writeLogIntoFile("Coping process started for credit note pdf file " + (start + 1) + " from " + end, logFileName, file.fileName, logFilePath, 'Purple')
        ...
        else
        {
            ...
            await uniqueFunction.writeLogIntoFile("Credit Note Pdf File Copied", logFileName, files[start-1].fileName, logFilePath, 'green')
