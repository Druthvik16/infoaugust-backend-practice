...
let apiName = '/api/vendorFileProcessController/getCreditNoteSummaryFileFromInputFolderLocal'
let processCreditNoteSummary = require('../scriptProcessing/processCreditNoteSummaryLocal')
const logFilePath = getPath.getName('vendor/s3/creditNote/summary')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'

let getCreditNoteSummary = {}
let clientID;

getCreditNoteSummary.getFileList = (client) => 
{
    ...
    try
    {
        ...
        if(start < end)
        {
            ...
            else
            {
                ...
                await uniqueFunction.writeLogIntoFile("Credit Note summary local folder is empty", logFileName, "*******************", logFilePath, 'green')
                start++;
    ...
    try
    {
        ...
        { 
            ...
            try
            {
                ...
                await uniqueFunction.writeLogIntoFile("Getting credit note summary file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('vendor/sftp/creditNote/summary'), 'Purple') 
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling credit note summary script", logFileName, file?.task.Key, logFilePath, 'green')
...
async function sendToCNSScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let processFile = await processCreditNoteSummary.getFileList(params.uploadCNSFile, params.inputFolderPath, params.encriptionIV, params.encriptionKey, params.client)
...
module.exports = getCreditNoteSummary
