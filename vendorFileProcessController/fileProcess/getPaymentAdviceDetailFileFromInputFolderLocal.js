...
let sourceFolder = 'Input_Payment_Advice_Detail_Raw_Sap_dump'
let clients;
let docPath = require('../../DOC_FOLDER_PATH/docPath')
let getPath = new docPath()
let logFileName = "paymentAdviceDetailLogFile-"
let apiName = '/api/vendorFileProcessController/getPaymentAdviceDetailFileFromInputFolderLocal'
let processPaymentAdviceDetail = require('../scriptProcessing/processPaymentAdviceDetailLocal')
const logFilePath = getPath.getName('vendor/s3/paymentAdviceDetail')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let clientID;

let getPaymentAdviceDetail = {}

getPaymentAdviceDetail.getFileList = (client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            logFileName = "paymentAdviceLogFile-"
...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Payment Advice Detail', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Payment Advice Detail', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Payment Advice Detail local folder is empty", logFileName, "*******************", logFilePath, 'green')
                start++;
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        { 
            ...
            try
            {
                console.log("Getting payment advice detail file for processing , file :" + (start + 1) + " from " + end)
                await uniqueFunction.writeLogIntoFile("Getting payment advice detail file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('vendor/sftp/paymentAdviceDetail'), 'Purple') 
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling payment advice detail script", logFileName, file?.task.Key, logFilePath, 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling payment advice detail script", file.task.Key)
                        let isFileSaved = await sendToPAScript(file)
                        if(isFileSaved)
                        {
                            start++;
                            return getFileObject(files, start, end, masterStart, masterEnd, res);
...
async function sendToPAScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let processFile = await processPaymentAdviceDetail.getFileList(params.uploadPAFile, params.inputFolderPath, params.encriptionIV, params.encriptionKey, params.client)
...
module.exports = getPaymentAdviceDetail
