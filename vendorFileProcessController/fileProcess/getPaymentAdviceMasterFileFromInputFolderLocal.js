...
let sourceFolder = 'Input_Payment_Advice_Master_Raw_Sap_dump'
let clients;
let docPath = require('../../DOC_FOLDER_PATH/docPath')
let getPath = new docPath()
let logFileName = "paymentAdviceMasterLogFile-"
let apiName = '/api/vendorFileProcessController/getPaymentAdviceMasterFileFromInputFolderLocal'
let processPaymentAdviceMaster = require('../scriptProcessing/processPaymentAdviceMasterLocal')
const logFilePath = getPath.getName('vendor/s3/paymentAdviceMaster')
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let clientID;

let getPaymentAdviceMaster = {}

getPaymentAdviceMaster.getFileList = (client) => 
{
...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Payment Advice Master', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Payment Advice Master', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Payment Advice Master local folder is empty", logFileName, "*******************", logFilePath, 'green')
                start++;
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        { 
            ...
            try
            {
                console.log("Getting payment advice master file for processing , file :" + (start + 1) + " from " + end)
                await uniqueFunction.writeLogIntoFile("Getting payment advice master file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('vendor/sftp/paymentAdviceMaster'), 'Purple') 
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling payment advice master script", logFileName, file?.task.Key, logFilePath, 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling payment advice master script", file.task.Key)
...
async function sendToPAScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let processFile = await processPaymentAdviceMaster.getFileList(params.uploadPAFile, params.inputFolderPath, params.encriptionIV, params.encriptionKey, params.client)
...
module.exports = getPaymentAdviceMaster
