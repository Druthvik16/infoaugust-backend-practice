...
const logFilePath = getPath.getName('vendor/s3/ledger');
const vendorModuleClientPath = '/' + uniqueFunction.vendorModule + 'client'
let clientID;

let getLedgerSummary = {}

getLedgerSummary.getFileList = (client) => 
{
...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Ledger', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Ledger', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Ledger summary local folder is empty", logFileName, "*******************", logFilePath, 'green')
                start++;
    ...
    try
    {
        ...
        { 
            ...
            try
            {
                ...
                await uniqueFunction.writeLogIntoFile("Getting ledger file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('vendor/sftp/ledger'), 'Purple') 
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling ledger summary script", logFileName, file?.task.Key, logFilePath, 'green')
