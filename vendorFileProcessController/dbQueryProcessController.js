    ...
    return new Promise((resolve, reject) => {
        try {
            let sql = `SELECT da.id, da.name 
                FROM vendor_document_attachment da 
                ORDER BY da.name`
            pool.query(sql, (error, result) => {
                if (error) {
                    return reject(error);
...
db.saveUploadDocLogMaster = (fileName, clientId, documentAttachmentId, status, uploadedOn, uploadedFilePath, documentCategoryId, encriptionKey, encriptionIV) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `INSERT INTO client_vendor_upload_doc_log_master (file_name, client_id, document_attachment_id, status, uploaded_file_path, uploaded_on, document_category_id,encryption_key,encryption_iv) VALUES ('${fileName}', '${clientId}', '${documentAttachmentId}', '${status}', '${uploadedFilePath}', ?, '${documentCategoryId}', '${encriptionKey}', '${encriptionIV}')`
            pool.query(sql, [uploadedOn], (error, result) => {
                if (error) {
                    return reject(error);
                }
...
db.saveUploadDocLogMasterLocal = (fileName, clientId, documentAttachmentId, status, uploadedOn, localFilePath, documentCategoryId, encriptionKey, encriptionIV) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `INSERT INTO client_vendor_upload_doc_log_master (file_name, client_id, document_attachment_id, status, local_file_path, uploaded_on, document_category_id,encryption_key,encryption_iv) VALUES ('${fileName}', '${clientId}', '${documentAttachmentId}', '${status}', '${localFilePath}', ?, '${documentCategoryId}', '${encriptionKey}', '${encriptionIV}')`
            pool.query(sql, [uploadedOn], (error, result) => {
                if (error) {
                    return reject(error);
                }
    ...
    return new Promise((resolve, reject) => {
        try {
            let sql = `SELECT dc.id, dc.name, dc.code 
                FROM vendor_document_category dc
        ...
        try {
            let sql = `SELECT IF(COUNT(cuvdm.id) > 0, 1, 0) AS isBillNoExist 
                FROM client_uploaded_vendor_doc_master cuvdm 
                    LEFT JOIN client_uploaded_vendor_doc_detail cuvdd ON cuvdd.client_uploaded_vendor_doc_master_id = cuvdm.id AND cuvdd.document_attachment_id = '${documentAttachmentId}'
                JOIN vendor_document_category vdc ON vdc.id = cuvdm.document_category_id AND vdc.name = 'Credit Note'
                WHERE cuvdd.client_uploaded_vendor_doc_master_id IS NULL 
    ...
    return new Promise((resolve, reject) => {
        try {
            let sql = `SELECT IF(COUNT(cuvdm.id) > 0, 1, 0) AS isBillNoExist 
                FROM client_uploaded_vendor_doc_master cuvdm 
                    LEFT JOIN client_uploaded_vendor_doc_detail cuvdd ON cuvdd.client_uploaded_vendor_doc_master_id = cuvdm.id AND cuvdd.document_attachment_id = '${documentAttachmentId}'
                JOIN vendor_document_category vdc ON vdc.id = cuvdm.document_category_id AND vdc.name = 'Form16A'
                WHERE cuvdd.client_uploaded_vendor_doc_master_id IS NULL ;`
            pool.query(sql, (error, result) => {
                if (error) {
                    return reject(error);
...
db.updatePdfBotProcessLog = (botName, isBillNoExist, isExecuteBot, isSummaryFileExist, isPdfFileExist) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE vendor_pdf_bot_process_log SET is_bot_execute = '${isExecuteBot}', is_bill_number_exist = '${isBillNoExist}', is_summary_file_exist = '${isSummaryFileExist}', is_pdf_file_exist = '${isPdfFileExist}' 
...
db.getUniqueProcessSequencetype = (clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `SELECT DISTINCT type FROM client_vendor_process_sequence_master  WHERE is_active = 1 AND client_id = '${clientId}';`
            pool.query(sql, (error, result) => {
                if (error) {
                    return reject(error);
                }
    ...
    return new Promise((resolve, reject) => {
        try {
            let sql = `WITH ActiveRows AS (
                        ...
                        FROM client_vendor_process_sequence_master
...
db.getProcessSequences = (id) => {
    ...
    pml.end_on AS endOn
FROM 
    client_vendor_process_sequence_master psm
LEFT JOIN 
    (
        SELECT 
            process_name, 
        ...
        FROM 
            client_vendor_process_module_log
        GROUP BY 
            process_name, 
            module_name
    ) latest_pml ON psm.name = latest_pml.process_name AND psm.type = latest_pml.module_name
LEFT JOIN 
    client_vendor_process_module_log pml ON latest_pml.process_name = pml.process_name 
...
db.updateProcessSequenceIsComplete = (isComplete, clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE client_vendor_process_sequence_master SET is_complete = '${isComplete}' where is_active = 1 AND client_id = '${clientId}';`
...
db.getProcessSequenceStatus = (clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `SELECT IF(COUNT(id) > 0,1,0) as working FROM client_vendor_process_sequence_master where status = 1 AND client_id = '${clientId}';`
...
db.updateProcessSequence = (status, isActive, processType, processSequenceName, isComplete, isError, clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE client_vendor_process_sequence_master SET status = '${status}', is_complete = '${isComplete}', is_error = '${isError}' `
...
db.updateProcessSequenceError = (processType, processSequenceName, isError) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE client_vendor_process_sequence_master SET is_error = '${isError}' `
...
db.savePreference = (id, isActive, readType, isError) => {
    return new Promise((resolve, reject) => {
        try {
            // , is_error = '${isError}' 
            let sql = `UPDATE client_vendor_process_sequence_master SET read_type = '${readType}' `
...
db.saveProcessSequenceLog = (processName, processType, date, clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `INSERT INTO client_vendor_process_module_log (process_name, module_name, start_on, client_id) VALUES 
...
db.updateProcessSequenceLog = (processName, processType, date, clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE client_vendor_process_module_log SET end_on = ? WHERE module_name = '${processType}' AND process_name = '${processName}' AND client_id = '${clientId}' ORDER BY id DESC LIMIT 1`
...
db.resetProcessSequence = (status, isComplete, isError, clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE client_vendor_process_sequence_master SET status = '${status}', is_complete = '${isComplete}', is_error = '${isError}' WHERE client_id = '${clientId}'`
...
db.updateInteruptProcess = (isWorking, modifyOn, clientId) => {
    return new Promise((resolve, reject) => {
        try {
            let sql = `UPDATE interupt_process_vendor SET is_working = '${isWorking}', modify_on = ? WHERE client_id = '${clientId}'`
