...
let sourceFolderSummary = 'Input_form16_Summary_Raw_Sap_dump'
let sourceFolderPdf = 'Input_form16_Pdfs_Raw_Sap_dump'
let isBillNoExist = 0;
...
let setForm16PdfBotStatus = {}

setForm16PdfBotStatus.getFileList = (client) => 
{
    return new Promise(async(resolve, reject) => 
    {
        ...
        {   
            ...
            {            
                ...
                checkBillNoExist = await db.checkBillNoExistForm16(documentAttachmentId[0]?.id)
...
async function checkSummaryLocalInputFolders(clients, start, end, res)
{
    try
    {
        ...
        if(start < end)
        {
            ...
            let localFilePathListObject = getPath.getName('documentFolders') + vendorModuleClientPath + "/"  + clientUuid + "/" + sourceFolderSummary + "/"
...
async function checkPdfLocalInputFolders(clients, start, end, res)
{
    try
    {
        if(start < end)
        {
            folderName = getPath.getName('documentFolders') + vendorModuleClientPath
            let clientUuid = clients[start]
            let localFilePathListObject = getPath.getName('documentFolders') + vendorModuleClientPath + "/"  + clientUuid + "/" + sourceFolderPdf + "/"
            let dirents = fs.readdirSync(localFilePathListObject, {withFileTypes : true});
            let files = dirents
            .filter(dirent => dirent.isFile())
            .map(dirent => {
...
async function msgToReturn()
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            if(!isPdfFileExist)
            {
                msg = msg + ` Bot cannot be started because form 16 pdf file not present. `
            }
        
            if(!isBillNoExist)
            {
                msg = msg + ` Bot cannot be started because form 16 number not present. `
            }
...
module.exports = setForm16PdfBotStatus