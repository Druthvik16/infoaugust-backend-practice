...
let copyForm16Summary = require('./fileSyncProcess/copyForm16SummaryFileFromSftpToLocal')
let copyCreditNotePdf = require('./fileSyncProcess/copyCreditNotePdfFileFromSftpToLocal')
let copyForm16Pdf = require('./fileSyncProcess/copyForm16PdfFileFromSftpToLocal')
let copyLedgerSummary = require('./fileSyncProcess/copyLedgerSummaryFileFromSftpToLocal')
let copyPaymentAdviceDetail = require('./fileSyncProcess/copyPaymentAdviceDetailFileFromSftpToLocal')
let copyPaymentAdviceMaster = require('./fileSyncProcess/copyPaymentAdviceMasterFileFromSftpToLocal')

let getCreditNoteSummary = require('./fileProcess/getCreditNoteSummaryFileFromInputFolderLocal')
let getForm16Summary = require('./fileProcess/getForm16SummaryFileFromInputFolderLocal')
let getLedgerSummary = require('./fileProcess/getLedgerSummaryFileFromInputFolderLocal')
let getPaymentAdviceDetail = require('./fileProcess/getPaymentAdviceDetailFileFromInputFolderLocal')
let getPaymentAdviceMaster = require('./fileProcess/getPaymentAdviceMasterFileFromInputFolderLocal')


let setCreditNotePdfBotStatus = require('./botProcess/setCreditNotePdfBotStatus')
let setForm16PdfBotStatus = require('./botProcess/setForm16PdfBotStatus')
...
async function executeProcessSequence(processType, sequence, readType, clientId, client)
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            {
                return resolve(await copyCreditNoteSummary.getFileList(readType, client))
            }
            else  if(sequence == 'CreditNotePdf' && processType == 'FileSync')
            {
                return resolve(await copyCreditNotePdf.getFileList(readType, client))
            }
            else if(sequence == 'Form16ASummary' && processType == 'FileSync')
            {
                return resolve(await copyForm16Summary.getFileList(readType, client))
            }
            else if(sequence == 'Form16APdf' && processType == 'FileSync')
            {
                return resolve(await copyForm16Pdf.getFileList(readType, client))
            }
            else  if(sequence == 'Ledger' && processType == 'FileSync')
            {
                return resolve(await copyLedgerSummary.getFileList(readType, client))
            }
            else  if(sequence == 'PaymentAdviceMaster' && processType == 'FileSync')
            {
                return resolve(await copyPaymentAdviceMaster.getFileList(readType, client))
            }
            else  if(sequence == 'PaymentAdviceDetail' && processType == 'FileSync')
            {
                return resolve(await copyPaymentAdviceDetail.getFileList(readType, client))
            }
            else if(sequence == 'CreditNoteSummary' && processType == 'FileProcess')
            {
                return resolve(await getCreditNoteSummary.getFileList(client))
            }
            else if(sequence == 'Form16ASummary' && processType == 'FileProcess')
            {
                return resolve(await getForm16Summary.getFileList(client))
            }
            else  if(sequence == 'Ledger' && processType == 'FileProcess')
            {
                return resolve(await getLedgerSummary.getFileList(client))
            }
            else  if(sequence == 'PaymentAdviceMaster' && processType == 'FileProcess')
            {
                return resolve(await getPaymentAdviceMaster.getFileList(client))
            }
            else  if(sequence == 'PaymentAdviceDetail' && processType == 'FileProcess')
            {
                return resolve(await getPaymentAdviceDetail.getFileList(client))
            }
            else  if(sequence == 'CreditNotePdf' && processType == 'BotProcess')
            {
                let response = await setCreditNotePdfBotStatus.getFileList(client)
                console.log(response)
                return resolve(response?.result)
            }
            else  if(sequence == 'Form16APdf' && processType == 'BotProcess')
            {
                let response = await setForm16PdfBotStatus.getFileList(client)
                console.log(response)
                return resolve(response?.result)
            }
        }
