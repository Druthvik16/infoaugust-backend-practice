...
let sourceFolder = 'Input_Invoice_Pt_File_Raw_Sap_dump'
let clients;
...
module.exports = require('express').Router().get('/',async(req,res) => 
{
    ...
    let processName = 'Invoice PT Summary'
    try 
    {
            /* ================= BLOCKER CHECK ================= */
        ...
        if(clients?.length == 0)
        {
            await uniqueFunction.writeLogIntoFile("Client List is empty", logFileName, "**************", getPath.getName('s3/invoice/pt'), 'red')
            res.status(500)
            return res.json({
                "status_code" : 500,
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "**************", getPath.getName('s3/invoice/pt'), 'red')
        console.log(e)
        res.status(500)
        return res.json({
            "status_code" : 500,
...
async function getListObjectFromS3Bucket(clientUuids, start, end, res)
{
    try
    {
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Invoice PT', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Invoice PT', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Invoice PT Local folder is empty", logFileName, "*******************", getPath.getName('s3/invoice/pt'), 'green')
                start++;
        ...
        {
            await uniqueFunction.writeLogIntoFile("All clients files passed to process api", logFileName, " ClientUUID : " + clientUuids[start-1], getPath.getName('s3/invoice/pt'), 'green')
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "ClientUUID : " + clientUuids[start], getPath.getName('s3/invoice/pt'), 'red')
        console.log(e)
        start++;
        getListObjectFromS3Bucket(clientUuids, start, end, res)
    }
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        { 
            ...
            try
            {
                console.log("Getting Invoice PT file for processing , file :" + (start + 1) + " from " + end)
                await uniqueFunction.writeLogIntoFile("Getting invoice PT file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('sftp/invoice/pt'), 'Purple') 
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Encryption key not present", logFileName, file?.task.Key, getPath.getName('s3/invoice/pt'), 'red')
                    start++;
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling invoice pt file script", logFileName, file?.task.Key, getPath.getName('s3/invoice/pt'), 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling Invoice PT script", file.task.Key)
                        let isFileSaved = await sendToInvoicePtScript(file)
                        if(isFileSaved)
                        {
                            start++;
                            getFileObject(files, start, end, masterStart, masterEnd, res);
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file not decrypted", logFileName, file?.task.Key, getPath.getName('s3/invoice/pt'), 'red')
                        start++;
                        getFileObject(files, start, end, masterStart, masterEnd, res);
                    }                    
                }
            ...
            {
                await uniqueFunction.writeLogIntoFile(fileError?.stack, logFileName, file['task']['Key'], getPath.getName('s3/invoice/pt'), 'red')
        ...
        {          
            await uniqueFunction.writeLogIntoFile("File send to API", logFileName, JSON.stringify(files[start-1]), getPath.getName('s3/invoice/pt'), 'green')
            masterStart++;
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "*********** " + "Start : " + start, getPath.getName('s3/invoice/pt'), 'red')
        console.log(e)
        start++
        getFileObject(files, start, end, masterStart, masterEnd, res)
    }
...
async function sendToInvoicePtScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let params = {
                uploadInvoicePtFile : fs.createReadStream("cron/"+fileName),
            ...
            axios.post(api.serviceApi + api.uploadedDoc + api.processInvoicePtLocal, params,
            {
                headers: 
                {
            ...
            then(async(response) => {
                params.uploadInvoicePtFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)
                await uniqueFunction.writeLogIntoFile(response?.data, logFileName, file?.task.Key, getPath.getName('s3/invoice/pt'), 'green')   
                return resolve(true)
            })
            .catch(async(err) => {
                params.uploadInvoicePtFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)  
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, file?.task.Key, getPath.getName('s3/invoice/pt'), 'red')  
        ...
        {
            await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, file?.task.Key, getPath.getName('s3/invoice/pt'), 'red')
            console.log(e)
            return resolve(true)
        }
    })
