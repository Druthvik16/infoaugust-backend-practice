...
{ 
    ...
    let processName = 'Credit Note Working'
    try
    {
        /* ================= BLOCKER CHECK ================= */
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        {     
            await uniqueFunction.writeLogIntoFile("File send to API", logFileName, JSON.stringify(files[start-1]), getPath.getName('s3/creditNote/working'), 'green')
            masterStart++;
...
async function sendToCNWScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let params = {
                uploadWorkingFile : fs.createReadStream("cron/"+fileName),
                client : JSON.stringify({"uuid": file?.clientUuid, "id": file?.clientId}),
                inputFolderPath : file.task.Key,
                encriptionKey : file.encriptionKey,
                encriptionIV : file.encriptionIV
            }
            axios.post(api.serviceApi + api.uploadedDoc + api.processCreditNoteWorkingLocal, params,
            {
                headers: 
                {
            ...
            then(async(response) => {
                params.uploadWorkingFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)    
                await uniqueFunction.writeLogIntoFile(response?.data, logFileName, file?.task.Key, getPath.getName('s3/creditNote/working'), 'green')
                return resolve(true)  
            })
            .catch(async(err) => {
                params.uploadWorkingFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)  
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, file?.task.Key, getPath.getName('s3/creditNote/working'), 'red')  
