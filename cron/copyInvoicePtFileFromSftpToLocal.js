            ...
            if(sftp.sftp)
            {
                ...
                try
                {
                    await sftp.list(dir).then(async(list) => 
                    {
                        ...
                        {
                            await uniqueFunction.writeLogIntoFile("Invoice PT Sftp server folder is empty, List Length : " + list?.length, logFileName, folderName, getPath.getName('sftp/invoice/pt'), 'red')
                            await sftp?.end()
...
async function getFileObject(files, start, end, res)
{
    try
    {
        if(start < end)
        {
            console.log("Coping process started for invoice PT file " + (start + 1) + " from " + end)
            ...
            //     await uniqueFunction.writeLogIntoFile("File name alrady exist in database", logFileName, file.fileName, getPath.getName('sftp/invoice/pt'), 'red')
                    ...
                    { 
                        ...
                        {                   
                            ...
                            else
                            {
                                ...
                                await uniqueFunction.writeLogIntoFile("File not encrypted error : " + JSON.stringify(encryptedData?.error), logFileName, file.fileName, getPath.getName('sftp/invoice/pt'), 'red')
                                start++
                                getFileObject(files, start, end, res)
                            }                                         
                        })
                        .catch(async (err) => {
                            await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, file.fileName, getPath.getName('sftp/invoice/pt'), 'red')
                            start++
                            getFileObject(files, start, end, res)
                        })
                    }
