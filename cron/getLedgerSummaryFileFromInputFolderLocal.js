...
let sourceFolder = 'Input_Ledger_Raw_Sap_dump'
let clients;
...
module.exports = require('express').Router().get('/',async(req,res) => 
{
    ...
    let processName = 'Ledger Note Summary'
    try
    {
        /* ================= BLOCKER CHECK ================= */
        ...
        if(clients?.length == 0)
        {
            await uniqueFunction.writeLogIntoFile("Client List is empty", logFileName, "**************", getPath.getName('s3/ledger'), 'red')
            res.status(500)
            return res.json({
                "status_code" : 500,
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "**************", getPath.getName('s3/ledger'), 'red')
        console.log(e)
        res.status(500)
        return res.json({
            "status_code" : 500,
...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Ledger Summary', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Ledger Summary', 'File Process completed', clientUuid, new Date())
                await uniqueFunction.writeLogIntoFile("Ledger summary local folder is empty", logFileName, "*******************", getPath.getName('s3/ledger'), 'green')
                start++;
        ...
        {
            await uniqueFunction.writeLogIntoFile("All clients files passed to process api", logFileName, " ClientUUID : " + clientUuids[start-1], getPath.getName('s3/ledger'), 'green')
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "ClientUUID : " + clientUuids[start], getPath.getName('s3/ledger'), 'red')
        console.log(e)
        start++;
        getListObjectFromS3Bucket(clientUuids, start, end, res)
    }
...
async function getFileObject(files, start, end, masterStart, masterEnd, res)
{
    try
    {
        ...
        { 
            ...
            try
            {
                console.log("Getting ledger summary file for processing , file :" + (start + 1) + " from " + end)
                await uniqueFunction.writeLogIntoFile("Getting ledger file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('sftp/ledger'), 'Purple') 
                ...
                {
                    await uniqueFunction.writeLogIntoFile("Encryption key not present", logFileName, file?.task.Key, getPath.getName('s3/ledger'), 'red')
                    start++;
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling ledger summary script", logFileName, file?.task.Key, getPath.getName('s3/ledger'), 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling ledger summary script", file.task.Key)
                        let isFileSaved = await sendToLedgerScript(file)
                        if(isFileSaved)
                        {
                            start++;
                            getFileObject(files, start, end, masterStart, masterEnd, res);
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file not decrypted", logFileName, file?.task.Key, getPath.getName('s3/ledger'), 'red')
                        start++;
                        getFileObject(files, start, end, masterStart, masterEnd, res);
                    }                    
                }
            ...
            {
                await uniqueFunction.writeLogIntoFile(fileError?.stack, logFileName, file['task']['Key'], getPath.getName('s3/ledger'), 'red')
        ...
        {          
            ...
            await uniqueFunction.writeLogIntoFile("File send to API", logFileName, JSON.stringify(files[start-1]), getPath.getName('s3/ledger'), 'green')
            masterStart++;
    ...
    {
        await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, "*********** " + "Start : " + start, getPath.getName('s3/ledger'), 'red')
        console.log(e)
        start++
        getFileObject(files, start, end, masterStart, masterEnd, res)
    }
...
async function sendToLedgerScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let params = {
                uploadLedgerFile : fs.createReadStream("cron/"+fileName),
            ...
            axios.post(api.serviceApi + api.uploadedDoc + api.processLedgerSummaryLocal, params,
            {
                headers: 
                {
            ...
            then(async(response) => {
                params.uploadLedgerFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)
                await uniqueFunction.writeLogIntoFile(response?.data, logFileName, file?.task.Key, getPath.getName('s3/ledger'), 'green')   
                return resolve(true)
            })
            .catch(async(err) => {
                params.uploadLedgerFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)  
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, file?.task.Key, getPath.getName('s3/ledger'), 'red')  
        ...
        {
            await uniqueFunction.writeLogIntoFile(e?.stack, logFileName, file?.task.Key, getPath.getName('s3/ledger'), 'red')
            console.log(e)
            return resolve(true)
        }
    })
