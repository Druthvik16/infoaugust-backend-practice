...
async function getListObjectFromS3Bucket(clientUuids, start, end,res)
{
    try
    {
        if(start < end)
        {
            ...
            if(files?.length > 0)
            {
                let saveDailyActivityLogDetail = await db.saveDailyActivityLogDetail('Process Local File', 'Partner', 'File processing started, No. of files available :' + files.length, clientUuid, new Date())
            ...
            else
            {
                ...
                let updateDailyActivityLogDetail = await db.updateDailyActivityLogDetailForClient('Process Local File', 'Partner', 'File Process completed', clientUuid, new Date())

                await uniqueFunction.writeLogIntoFile("Partner list Local folder is empty", logFileName, "*******************", getPath.getName('s3/partner'), 'green')
                start++;
    ...
    try
    {
        ...
        { 
            ...
            try
            {
                ...
                await uniqueFunction.writeLogIntoFile("Getting credit note summary file for processing , file :" + (start + 1) + " from " + end, logFileName, file.task.Key.split('/').pop(), getPath.getName('sftp/partner'), 'Purple') 
                ...
                else
                {
                    ...
                    {
                        await uniqueFunction.writeLogIntoFile("file decrypted, Calling partner onboarding list script", logFileName, file?.task.Key, getPath.getName('s3/partner'), 'green')
                        file.file['Body'] = decryptedData?.file
                        console.log("Calling partner onboarding script", file.task.Key)
...
async function sendToPartnerUploadScript(file) 
{
    return new Promise(async(resolve, reject) => 
    {
        try
        {
            ...
            let params = {
                partnerFile : fs.createReadStream("cron/"+fileName),
            ...
            axios.post(api.serviceApi + api.partner + api.uploadPartnerFromSftpToLocal, params,
            {
                headers: 
                {
            ...
            then(async(response) => {
                params.partnerFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)
                await uniqueFunction.writeLogIntoFile(response?.data, logFileName, file?.task.Key, getPath.getName('s3/partner'), 'green')  
                return resolve(true) 
            })
            .catch(async(err) => {
                params.partnerFile?.close();
                uniqueFunction.removeFileFromDirectory("cron/"+fileName)  
                await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, file?.task.Key, getPath.getName('s3/partner'), 'red')  
