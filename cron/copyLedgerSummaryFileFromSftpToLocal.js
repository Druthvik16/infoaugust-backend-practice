...
async function getFileObject(files, start, end, res)
{
    try
    {
        if(start < end)
        {
            console.log("Coping process started for ledger file " + (start + 1) + " from " + end)
            ...
            //     await uniqueFunction.writeLogIntoFile("File name alrady exist in database", logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
                    ...
                    { 
                        ...
                        {                   
                            ...
                            else
                            {
                                ...
                                await uniqueFunction.writeLogIntoFile("File not encrypted error : " + JSON.stringify(encryptedData?.error), logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
                                start++
                                getFileObject(files, start, end, res)
                            }                                         
                        })
                        .catch(async (err) => {
                            await uniqueFunction.writeLogIntoFile(err?.stack, logFileName, file.fileName, getPath.getName('sftp/ledger'), 'red')
                            start++
                            getFileObject(files, start, end, res)
                        })
                    }
