...
createRawData.post('/', async (req, res) => {
    try {
        ...
        const result = await db.getS3FileDetails(financialYear, clientUuid);
        ...
        const ledgerFiles = listedObjects.Contents

        // Step 2: Rename previous ledger.json if it exists
        const oldFileKey = file_path;
        const newFileKey = `${prefix}/${fileName}_${ledgerFiles.length}.json`;

        const fileExists = ledgerFiles.some(obj => obj.Key === oldFileKey);
        ...
        res.status(200).send({
            message: 'ledger.json created and old file (if any) renamed successfully.',
            newBackupFile: newFileKey
        });

    } catch (err) {
        console.error(err);
        res.status(500).send({ error: 'Failed to create or rename ledger file.' });
    }
});
