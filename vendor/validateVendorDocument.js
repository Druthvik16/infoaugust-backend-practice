...
module.exports = require('express').Router().post('/',async(req,res) =>
{
    try
    {
        ...
        else  if(vendorStatusName == 'Document-Submitted')
        {
            ...
            {
                let validateVendorDocument = await db.validateVendorDocument(vendorDocumentId, new Date(), isInfomapVerified, userId)
                if(validateVendorDocument.affectedRows > 0)
                {
                    res.status(200)
                    return res.json({
                        "status_code" : 200,
